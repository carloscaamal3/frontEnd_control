<template>
  <v-container id="data-tables" tag="section">
    <base-material-card color="blue" icon="mdi-file-document-edit-outline" inline class="px-5 py-3">
      <v-spacer></v-spacer>
                ...
                <v-row>
                  ...
                  <v-col cols="12">
                    <div class="right" v-if="this.saldoxafectar - editedItem.sp_importe > 0">
                      <v-checkbox v-model="checkboxSaldo" label="Devolver Efectivo ?"></v-checkbox>
                    </div>
                  </v-col>
                </v-row>
              </v-container>
      ...
      <v-dialog v-model="dialogxc" persistent max-width="800px">
        <v-card class="v-text">
          ...
          <v-card-actions class="v-text">
            ...
            <v-btn color="blue darken-1" text @click="Borrar()" :disabled="validado">Guardar</v-btn>
          </v-card-actions>
        </v-card>
      </v-dialog>
 ...
 */
        params =
          `(sp_estatus ='CREADO' or sp_estatus = 'XCANCELAR' or sp_estatus = 'PRECAP') and sp_ejercicio = ` +
          ejercicio +
          `/sp_id/`
      }
