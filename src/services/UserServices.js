...
export default {
   ...
   */
 async postUser(jwt, body) {
    try {
      apiUsers.defaults.headers.common['x-token'] = jwt;
      const response = await apiUsers.post('usuarios' , body);
   ...
   */
  async patchUser(jwt, body, id) {
    try {
      apiUsers.defaults.headers.common['x-token'] = jwt;
      const response = await apiUsers.put('usuarios/' + id, body);
      return response.data;
    } catch (error) {
      console.error('Error al crear el registro de ciudadano:', error);
      throw new Error('No se pudo crear el registro de ciudadano');
  ...
  getUsersId(jwt, id) {
    apiUsers.defaults.headers.common['x-token'] = jwt;
    return apiUsers.get('usuarios/' + id)
  },
  ...
  async postCambioPwd(jwt, body, id) {
  try {
